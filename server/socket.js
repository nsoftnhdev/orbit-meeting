...
export function setupSocketIO(io) {
  io.on("connection", (socket) => {
    ...
    socket.on(
      ...
      async ({ roomId, user, audioEnabled = true, videoEnabled = true }) => {
        ...
        AND ((${userId}::text IS NOT NULL AND user_id = ${userId}) OR name = ${currentUser.userName})`;
          ...
          VALUES (${meeting.id}, ${user.id}, ${currentUser.userName}, NOW())`;
          }
    ...
    // Passes the connection details from one user to the other so WebRTC can figure out how to connect the directly.
    socket.on(
      "ice-candidate",
